...
def test_get_client(minio_resource):
    ...
    with patch("services.dagster.etl_pipelines.resources.minio_resource.Minio") as mock_minio:
...
def test_list_manifests_returns_json_files_only(minio_resource):
    ...
    with patch("services.dagster.etl_pipelines.resources.minio_resource.Minio") as mock_minio:
        mock_client = Mock()
        mock_client.list_objects.return_value = [mock_obj1, mock_obj2, mock_obj3, mock_obj4]
...
def test_list_manifests_raises_on_missing_bucket(minio_resource):
    ...
    with patch("services.dagster.etl_pipelines.resources.minio_resource.Minio") as mock_minio:
        ...
        with pytest.raises(RuntimeError, match="Landing bucket 'test-landing' does not exist"):
...
def test_get_manifest_raises_on_missing_object(minio_resource):
    ...
    with patch("services.dagster.etl_pipelines.resources.minio_resource.Minio") as mock_minio:
        ...
        with pytest.raises(RuntimeError, match="Manifest 'manifests/missing.json' not found"):
...
def test_move_to_archive_copies_and_deletes(minio_resource):
    ...
    with patch("services.dagster.etl_pipelines.resources.minio_resource.Minio") as mock_minio:
        ...
        mock_client.copy_object.assert_called_once_with(
            "test-landing",
            "archive/manifests/batch_123.json",
            "test-landing/manifests/batch_123.json",
        )
...
def test_upload_to_lake_uploads_file_with_inferred_content_type(minio_resource, tmp_path):
...
def test_upload_to_lake_uploads_file_with_explicit_content_type(minio_resource, tmp_path):
...
def test_upload_to_lake_raises_on_missing_file(minio_resource):
    ...
    with pytest.raises(FileNotFoundError, match="Local file not found"):
        minio_resource.upload_to_lake("/nonexistent/file.parquet", "datasets/test.parquet")
