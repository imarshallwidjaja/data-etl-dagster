...
@asset(
    ...
    description="Validates and normalizes manifest JSON from run config.",
)
def raw_manifest_json(context: AssetExecutionContext, config: ManifestConfig) -> Dict[str, Any]:
    """
    Root asset that validates and normalizes manifest JSON.
    
    Receives manifest dict from run config (set by sensor) and validates it
    against the Manifest model. Outputs normalized dict for use by downstream assets.
    ...
    Returns:
        Normalized manifest dict (via model_dump(mode="json"))
