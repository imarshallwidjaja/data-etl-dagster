"""Complex spreadsheet sensor.

Polls ``landing-zone/manifests/`` for ``ingest_complex_spreadsheet`` manifests
and triggers ``complex_table_splitter_job`` to split multi-table XLSX files.
"""
...
@sensor(
    ...
    name="complex_spreadsheet_sensor",
    description="Polls for complex spreadsheet manifests (ingest_complex_spreadsheet) and triggers complex_table_splitter_job",
)
def complex_spreadsheet_sensor(context: SensorEvaluationContext, minio: MinIOResource):
    try:
    ...
    for manifest_key in new_manifests:
        try:
            ...
            if manifest.intent != "ingest_complex_spreadsheet":
                continue
            ...
            # Op-based job: pass manifest via init_mongo_run_op inputs
            run_request = RunRequest(
                ...
                tags={
                    ...
                    "sensor": "complex_spreadsheet_sensor",
                    "partition_key": partition_key,
            ...
            context.log.info(
                f"Triggered complex_table_splitter_job for manifest '{manifest_key}' "
